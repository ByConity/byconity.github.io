...
Firstly, we need to download the binary for the installation in [here](https://github.com/apple/foundationdb/releases/). We need to download the **server**, **monitor** and **cli** binaries, and those corresponding **sha256** checksum files. I will choose version **7.1.25** as it is the latest at the time.\
