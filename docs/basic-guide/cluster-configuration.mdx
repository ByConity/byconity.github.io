...
- storage_configuration
- cnch_default_policy, specifies the StoragePolicy used by CNCH to store actual data, an optional configuration item, the default is cnch_default_hdfs, and this StoragePolicy should only include HDFSDisk
- When creating a table, for a CNCH table, if the storage_policy MergeTreeSettings is not specified, it will be changed to ${cnch_default_policy} by default
- cnch_auxility_policy, specifies the StoragePolicy used by CNCH to store temporary data on the local disk, optional configuration item, the default is default
- disks
- ${DISK_NAME}
- type, the type of this disk, optional configuration item, the default is local
- CNCH supports the selection of two disks, hdfs/bytehdfs, as remote storage. Currently, only a single Disk is supported in the StoragePolicy
...
- The configuration items of HDFSDisk specified by the StoragePolicy used by CNCH should be the same in all Server/Worker configuration files
- path
...
After configuration, CNCH will open kafka_log, and the consumption log can be viewed through the system table
...
The thread pool size of CNCH background cleaning transaction record, the default is 128
...
The thread pool queue size of CNCH background cleaning transaction record, the default is 10000
