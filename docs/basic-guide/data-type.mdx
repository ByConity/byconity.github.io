...
To generate UUID values, ByConity provides the [generateuidv4](https://clickhouse.com/docs/zh/sql-reference/functions/uuid-functions) function.
...
You can see more rules about **NaN** sorting in [ORDER BY clause](https://clickhouse.com/docs/zh/sql-reference/data-types/float) 
...
Please use the [UUID](https://clickhouse.com/docs/zh/sql-reference/data-types/uuid) data type to store UUID values.
...
ByConity will automatically detect the array elements, and calculate the smallest data type to store these elements according to the elements. If there is a [NULL](https://clickhouse.com/docs/zh/sql-reference/data-types/array#null-literal) or a [null-literal](https://clickhouse.com /docs/zh/sql-reference/data-types/nullable#data_type-nullable) type element, then the element type of the array will become [nullable](https://clickhouse.com/docs/zh/sql -reference/data-types/nullable).
...
Allow special markup ([NULL](https://clickhouse.com/docs/zh/sql-reference/data-types/nullable)) to indicate «missing value», which can be stored together with the normal value of **TypeName** . For example, a column of type **Nullable(Int8)** can store a value of type **Int8**, while a row with no value will store **NULL**.
...
A family of data types representing time and date intervals. [INTERVAL](https://clickhouse.com/docs/zh/sql-reference/operators/#operator-interval) The result type of the operation.
...
You can use [date](https://clickhouse.com/docs/zh/sql-reference/data-types/date) and [datetime](https://clickhouse.com/docs/zh/sql- reference/data-types/datetime) values of type **Interval** are used in arithmetic operations on values of type **Interval**. For example, you can add 4 days to the current time:
...
For example, the literal [NULL](https://clickhouse.com/docs/zh/sql-reference/syntax#null-literal) has type **Nullable(Nothing)**. See Nullable for details.
