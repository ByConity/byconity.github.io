...
| lru_max_size | Maximum size of the LRU cache (in bytes) | 2T |
| lru_max_object_num | Maximum number of objects in the LRU cache | std::numeric_limits<size_t>::max() |
...
| lru_update_interval | LRU cache update interval (in seconds) | 60s |
